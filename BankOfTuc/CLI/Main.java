 ...
 */
    ...
    /* private static void countdownCLI(User user, LoginManager login) {
        stopCountdownTimer();
        ...
        currentTimerThread = new Thread(() -> {
            try {
                ...
                while (timerRunning) {
                    long remainingMs = login.getTimeRemaining(user.getUsername());
                    if (remainingMs <= 0) {

                        System.out.print("\u001B[1A");
                        System.out.print("\rTime: TIME'S UP!          ");
                        System.out.print("\u001B[1B"); 
                        timerRunning = false;
                        break;
                    }
