...
public class TransactionPdfGenerator {
    ...
    String html = """
        ...
        """.formatted(
            LOGO_BASE64,          // %s → logo
            entry.datetime,       // %s → date
            entry.type,           // %s → type
            orderingCustomer,     // %s → ordering customer
            orderingAccount,      // %s → ordering account
            recipientCustomer,    // %s → recipient customer
            (entry.type.equals("PAYMENT") ? "RF code" : "IBAN Παραλήπτη"),
            (entry.type.equals("PAYMENT") ? entry.rfCode : entry.counterpartyIban),
            entry.isOutgoing ? "amount out" : "amount in", // %s → amount class
            amountDisplay         // %s → amount display
        );
